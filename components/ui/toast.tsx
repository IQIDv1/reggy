...
import { useToastPosition } from "./toast-context";
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <ToastPrimitives.Viewport
      ref={ref}
      className={cn(
        "z-[100] flex max-h-screen w-full p-4 sm:max-w-[420px]",
...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full",
  {
    variants: {
      ...
      position: {
        "top-right": "data-[state=open]:slide-in-from-top-full",
        "top-left": "data-[state=open]:slide-in-from-top-full",
        "bottom-right": "data-[state=open]:slide-in-from-bottom-full",
        "bottom-left": "data-[state=open]:slide-in-from-bottom-full",
...
>(({ className, variant, position, ...props }, ref) => {
  const { position: contextPosition } = useToastPosition();
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={cn(
        toastVariants({ variant, position: position || contextPosition }),
        className
      )}
      {...props}
    />
